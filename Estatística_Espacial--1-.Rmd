...
$Obs_1$ Os pontos em geral não estão associados a valores, mas apenas à ocorrência
dos eventos considerados.

$Obs_2$ Em alguns estudos os pontos podem estar associados a atributos de
identificação.
...
$Obs_1$ A plotagem dos resultados desta função de distribuição cumulativa empírica $\hat G(h)$ pode ser  para se verificar se existe evidência de interação entre os eventos. 

$Obs_2$ O método de análise de vizinhança pode ser usada para se comparar estatísticamente a distribuição dos eventos observados com o que se esperaria na hipótese da aleatoriedade espacial completa (CSR). Para verificar a condição de aleatoriedade, calculamos os envelopes de simulação superior e inferior. 

$Obs_3$ o método do vizinho mais próximo fornece uma indicação inicial da distribuição espacial, tem a desvantagem por considera apenas escalas pequenas. Para se ter informação mais efetiva para o padrão espacial em escalas maiores, pode ser recomendado usar é o métdo da função K
...
- $Obs_1$ A função K é usada como ferramenta exploratória na comparação entre estimativa empírica — $\hat K(h)$ — e a resultante de um processo de padrão de pontos
espacial aleatório — $K(h)$, sendo que se estamos completamente aleátorio $K(h) = h\pi^2$

- $Obs_2$ uma forma de comparar a estimativa K de um conjunto de dados observados com $h\pi^2$ seria plotar a função $\hat L(h)$ definida como $\hat L(h) = \sqrt\frac{\hat K(h)-h}{\pi}$. A abordagem é similar à do vizinho mais próximo, pode ser feita para se estimar a significância dos desvios da distribuição $\hat L(h)$ em relação à condição de aleatoriedade (CSR)
...
dados_shape_RN_BDqueimadas = read_excel("focos_qmd_inpe_2023-11-30_2024-10-01_08.627347.xls")
...
```{r, include=T, menssage = F, warning=F, echo=F}
#AD <- describe(dados_shape_RN_BDqueimadas[, c(8,9,12)]); AD_df_transposed  = t(AD[,-c(1,2)])
#rownames(AD_df_transposed) <- c("Média", "Desvio padrão", "Mediana", "Média Truncada - Corte10%","Desvio absoluto mediano", "Mínimo","Máximo","Amplitude",  "Coef. Assimétria", "Kurtose","Erro padrao")
...
j1 = ggplot(dados_shape_RN_BDqueimadas,
            aes(x = Bioma, y = FRP)) +
  xlab("Biomas") +
  ylab("Potência Radiativa do Fogo em megawatt s (MW)") +
  geom_boxplot(width = 0.7, show.legend = FALSE) +
  geom_jitter(position = position_jitter(0.2), show.legend = FALSE) +
  labs(
    title = "Distribuição via boxplot",
    subtitle = "Potência Radiativa do Fogo por bioma"
  ) +
...
j2 = ggplot(dados_shape_RN_BDqueimadas,
            aes(x = Bioma, y = Precipitacao )) +
  xlab("Biomas") +
  ylab("Precipitação em milímetros (mm)") +
  geom_boxplot(width = 0.7, show.legend = FALSE) +
  geom_jitter(position = position_jitter(0.2), show.legend = FALSE) +
  labs(
    title = "Distribuição via boxplot",
    subtitle = "Precipitação por bioma"
  ) +
  theme_bw(base_size = 20)
...
j3 = ggplot(dados_shape_RN_BDqueimadas,
  ...
  labs(
    title = "Distribuição via boxplot",
    subtitle = "Risco de fogo por bioma"
  ) +
  theme_bw(base_size = 20)
...
dados_FRP <- data.frame(
  Estado = rep("RN", length(FRP_filtrado)),
  FRP = FRP_filtrado
)

f1 <- ggplot(dados_FRP, aes(x = FRP, y = Estado)) +
  geom_boxplot(width = 0.7, show.legend = FALSE,
               fill = "orange", outlier.colour = "red", outlier.shape = 1) +
  ylab("") +
  xlab("Potência de Energia Liberada em megawatts (MW)") +
...
dados_RiscoFogo <- data.frame(
  Estado = rep("RN", length(RiscoFogo_filtrado)),
  RiscoFogo = RiscoFogo_filtrado
)

f2 <- ggplot(dados_RiscoFogo, aes(x = RiscoFogo, y = Estado)) +
  geom_boxplot(width = 0.7, show.legend = FALSE,
               fill = "orange", outlier.colour = "red", outlier.shape = 1) +
  ylab("") +
  xlab("Risco de Fogo") +
...
dados_precipitacao <- data.frame(
  Estado = rep("RN", length(Precipitacao_filtrado)),
  Precipitacao = Precipitacao_filtrado
)

f3 <- ggplot(dados_precipitacao, aes(x = Precipitacao, y = Estado)) +
  geom_boxplot(width = 0.7, show.legend = FALSE,
               fill = "orange", outlier.colour = "red", outlier.shape = 1) +
  ylab("") +
  xlab("Precipitação em milímetros (mm)") +
  labs(title = "Distribuição via Boxplot") +
  scale_x_continuous(limits = c(0,0.6), breaks = seq(0,0.6,0.05)) +
  theme(axis.text.x = element_text(hjust = 1, size = 12),
        axis.title.y = element_text(size = 12),
